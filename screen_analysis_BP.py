...
def count_spacers(fastq_file, output_file, countDict, guideGeneDict, geneCountDict):
    ...
    print(len(list(countDict.values())))
    ...
    # print stats out to *.csv
    outputName = str(output_fie+"_statistics.csv")
    with open(outputName, 'w') as csvfile:
        mywriter = csv.writer(csvfile, delimiter=',');
        ...
        mywriter.writerow(["Number of reads processed", num_reads]);
...
def calcGeneEnrich(unsortedGeneCountDict, unsortedTotMatches, sortedGeneCountDict, sortedTotMatches, descriptionDict):
    ...
    for counter, key in enumerate(keys):
        ...
        unsorted_count = int(unsortedGeneCountDict[key])
        sorted_count = int(sortedGeneCountDict[key])
        fold_change = math.log2(((sorted_count+1)/sortedTotMatches)/((unsorted_count+1)/unsortedTotMatches))
        print(unsorted_count, sorted_count, fold_change)
...
class parseThread(threading.Thread):
    def __init__(self, sorted, unsorted, output, guides):
        self.sorted = sorted
